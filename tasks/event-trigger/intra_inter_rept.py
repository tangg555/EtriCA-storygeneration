...
from src.utils.event_trigger.eval_utils import eval_intra_inter_repetitions
from src.utils import nlg_eval_utils
from src.utils.event_trigger import visual_utils

# roc stories -------------------
roc_reference_intra = ({1: 0.0, 2: 1.28, 3: 1.72, 4: 2.03, 5: 2.18}, 1.8)
roc_reference_inter = ({1: 26.91, 2: 17.28, 3: 15.22, 4: 13.97}, 18.34)
# planaw
roc_leading_planaw_intra = ({1: 0.0, 2: 0.47, 3: 1.24, 4: 1.34, 5: 1.45}, 1.12)
roc_leading_planaw_inter = ({1: 26.91, 2: 20.66, 3: 19.31, 4: 17.87}, 21.19)
roc_event_planaw_intra = ({1: 0.0, 2: 0.25, 3: 1.25, 4: 1.55, 5: 1.52}, 1.14)
roc_event_planaw_inter = ({1: 26.91, 2: 20.59, 3: 19.01, 4: 18.06}, 21.14)
roc_leading_plus_event_planaw_intra = ({1: 0.0, 2: 0.51, 3: 1.25, 4: 1.52, 5: 1.5}, 1.2)
roc_leading_plus_event_planaw_inter = ({1: 26.91, 2: 20.3, 3: 18.54, 4: 18.4}, 21.04)
# gpt2
...
def get_rept(dataset_name=None, model_name=None, file_prefix=None):
    ...
    parser.add_argument('--gen_file', type=str,
                        default=f"output/event-trigger/{model_name}-{dataset_name}/"
                                f"gen_result/{file_prefix}.source_gen.txt",
                        help='path of file containing generated text')
    args = parser.parse_args()

    data_dir = Path(f"{BASE_DIR}/datasets/event-trigger/{args.dataset_name}")
    src_file = data_dir.joinpath(f"test.source.txt")
    # hint特殊，因为有相同的输入
