...
def load_event_predictor(dataset_name) -> EventPredictor:
    predictor = EventPredictor(name=dataset_name,
                               event_extractor_path=f"{BASE_DIR}/output/event-trigger/cache/{dataset_name}_event_graph.pkl",
                               cache_dir=f"{BASE_DIR}/output/event-trigger/cache",
                               data_dir=f"{BASE_DIR}/resources/datasets/event-trigger/{dataset_name}",
                               output_dir=f"{BASE_DIR}/resources/datasets/event-trigger/{dataset_name}")
    return predictor
...
def retrieve_gen_result(dir_key_words:list,
                        ...
                        result_dir=f'{BASE_DIR}/output/event-trigger',
                        output_dir=f'{BASE_DIR}/output/event-trigger/gen_result',
...
if __name__ == '__main__':
    ...
    # for etrica ===============
    discard_dir_key_words = ["gpt", "planaw"]
    discard_file_key_words = ["predicted", "bart"]
    output_dir = f'{BASE_DIR}/output/event-trigger/gen_result_for_etrica'
    # roc stories
